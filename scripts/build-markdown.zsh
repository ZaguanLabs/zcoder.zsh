...
typeset dependency=$project_root/vendor/zmdown
...
ZMDOWN_BUILD_NAME= make -C "$dependency" build
# Verify the module loads in this shell ABI, isolated from the build process.
zsh -dfc 'module_path=("$1" $module_path); zmodload zmdown' \
  zcoder-markdown "$dependency/.build/modules"
print -r -- "$signature" >| "$dependency/.build/zcoder-abi"
print -r -- 'Local zmdown enabled. Run make test to exercise Markdown rendering.'
