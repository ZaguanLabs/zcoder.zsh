...
assert_contains "$REPLY" "wait for that result before issuing it" "sysadmin prompt also separates dependent tool calls"
assert_contains "$REPLY" "No text matches does not prove a named file is absent" "sysadmin prompt receives shared evidence rules"
...
assert_contains "$REPLY" "wait for that result before issuing it" "system prompt separates dependent calls across responses"
assert_contains "$REPLY" "remains valid if another call fails" "system prompt limits batches to independent calls"
assert_not_contains "$REPLY" "Put a prerequisite before" "system prompt removes unsafe dependent batching guidance"
assert_contains "$REPLY" "No text matches does not prove a named file is absent" "all coding models receive search evidence limits"
assert_contains "$REPLY" "copy it verbatim without shortening or normalizing it" "all coding models preserve exact observed values"
assert_contains "$REPLY" "Preserve unrelated user changes" "coding prompt protects existing user work"
...
assert_contains "$REPLY" "Before completing, check the requested outcome" "system prompt requires evidence before completion"
...
assert_contains "$REPLY" "copy it verbatim without shortening or normalizing it" "LFM prompts preserve exact observed values"
assert_contains "$REPLY" "search finds text inside files, not filenames" "LFM prompts distinguish content search from file discovery"
assert_contains "$REPLY" "Before editing an existing file, read the relevant source" "LFM prompts require edit evidence before patching"
