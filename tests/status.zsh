...
# Check the narrow layout, both sides of the 100-column budget change, and
# a wide layout. Busy-phase normalization is covered separately below.
() {
  ...
  local phase='' call='' identity='' baseline='' badge_column='' badge=''
  for SCREEN_W in 40 99 100 160; do
    baseline=''
    for phase in Ready 'Error: long diagnostic that must not cover the project'; do
      UI_NOTICE_UNTIL=0
      ui_set_status "$phase"; ui_status_update
      MOCK_ZCURSES_CALLS=(); _ui_paint_header 1
      identity=''; badge_column=''; badge=''
      for call in "${MOCK_ZCURSES_CALLS[@]}"; do
        ...
        elif [[ $call == 'string top_win [ '* ]]; then
          badge=${call#string top_win }
        fi
      done
      [[ $phase == Ready ]] && baseline=$identity
      assert_eq "$baseline" "$identity" "$SCREEN_W columns: $phase leaves the header identity unchanged"
      assert_eq "$(( SCREEN_W - 2 ))" "$(( badge_column + ${(m)#badge} ))" "$SCREEN_W columns: $phase keeps the closing bracket at the right edge"
      assert_not_contains "$badge" '  ' "$SCREEN_W columns: $phase keeps padding outside the brackets"
    done
  done
}
...
test_integration status || return 0
...
assert_contains "${mapfile[$status_pty_base.header]:-}" '[ Working ]' "real curses keeps the status brackets close to the text"
