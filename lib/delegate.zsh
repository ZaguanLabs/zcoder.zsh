# Read-only consultations with external coding harnesses.
...
typeset -g ZCODER_AGY_MODEL="${ZCODER_AGY_MODEL:-gemini-3.7-flash-medium}"
...
typeset -g DELEGATE_MODEL="" DELEGATE_OUTPUT="" DELEGATE_ERROR=""
...
delegate_build_command() {
  local provider="$1" request="$2" model=""
  local consultation=""
  DELEGATE_COMMAND=()
  DELEGATE_STDIN_PROMPT=0
  DELEGATE_MODEL=""
  DELEGATE_ERROR=""
  delegate_consultation_prompt "$request"; consultation="$REPLY"
  ...
  case "$provider" in
    claude)
      ...
      DELEGATE_COMMAND=(claude -p --model "$model" --effort medium
        ...
        --no-session-persistence --disable-slash-commands -- "$consultation")
      ;;
    codex)
    ...
    agy)
      ...
      DELEGATE_COMMAND=(agy -p --model "$model" --effort medium
        --output-format json --mode plan --sandbox --disable-slash-commands
        -- "$consultation")
      ;;
    opencode)
...
delegate_async_start() {
  ...
  if [[ -n "$DELEGATE_PID" ]] && kill -0 "$DELEGATE_PID" 2>/dev/null; then
    DELEGATE_ERROR="another delegated consultation is already running"
    return 1
  fi
...
delegate_async_cancel() {
  ...
  DELEGATE_OUTPUT=""
  DELEGATE_ERROR="delegated consultation cancelled"
}
...
delegate_async_collect() {
  ...
  DELEGATE_OUTPUT=""
  DELEGATE_ERROR=""
  [[ -n "$base" ]] || { DELEGATE_ERROR="no delegated consultation is running"; return 1; }
  if [[ -f "${base}.done" ]]; then
...
delegate_discover_opencode_models() {
  ...
  DELEGATE_ERROR=""
  [[ -n "${commands[opencode]:-}" ]] || { DELEGATE_ERROR="opencode is not installed or is not on PATH"; return 127; }
...
delegate_run() {
  local provider="$1" request="$2" binary="" label="" result="" stderr=""
  ...
  binary="$REPLY"
  [[ -n "${commands[$binary]:-}" ]] || { DELEGATE_ERROR="$binary is not installed or is not on PATH"; return 127; }
  delegate_build_command "$provider" "$request" || return $?
  delegate_label "$provider"; label="$REPLY"

  if (( ${UI_ACTIVE:-0} && $+functions[ui_append_message] )); then
    ui_append_message user "/${provider} ${request}"
    ui_set_status "${label} consulting"
    ui_refresh_all
  fi
  zcoder_debug delegate_start "provider=$provider model=${(qqq)DELEGATE_MODEL} request=${(qqq)request}"
  delegate_consultation_prompt "$request"
  if ! delegate_async_start "$REPLY" "$DELEGATE_STDIN_PROMPT" "${DELEGATE_COMMAND[@]}"; then
    (( ${UI_ACTIVE:-0} )) && ui_append_message error "${label} failed: ${DELEGATE_ERROR:-could not start the CLI}"
  ...
  if (( wait_status == 130 )); then
    delegate_async_cancel
    (( ${UI_ACTIVE:-0} )) && ui_append_message system "⏹ ${label} consultation stopped."
    (( ${UI_ACTIVE:-0} )) && ui_set_status "Stopped"
    zcoder_debug delegate_cancel "provider=$provider"
    return 130
  elif (( wait_status == 124 )); then
    delegate_async_cancel
    DELEGATE_ERROR="${label} consultation timed out after ${ZCODER_DELEGATE_TIMEOUT_SECONDS} seconds"
  ...
  if (( request_status != 0 )); then
    ...
    (( ${UI_ACTIVE:-0} )) && ui_append_message error "${label} failed: ${DELEGATE_ERROR}"
    (( ${UI_ACTIVE:-0} )) && DELEGATE_ERROR_REPORTED=1
    (( ${UI_ACTIVE:-0} )) && ui_set_status "Delegate error"
  ...
  result="$REPLY"
  delegate_remember "$provider" "$DELEGATE_MODEL" "$request" "$result"
  if (( ${UI_ACTIVE:-0} )); then
    ui_append_message "$provider" "$result"
    ui_set_status "Ready"
    ui_refresh_all
  else
    print -r -- "$result"
  fi
  zcoder_debug delegate_complete "provider=$provider model=${(qqq)DELEGATE_MODEL} chars=${#result}"
}
